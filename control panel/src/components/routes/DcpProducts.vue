<template>
  <div class="blue-grey lighten-5 container-dashboard">
    <v-container fluid>
      ...
      <v-row>
        <v-col v-for="(categorie, i) in categories" :key="i" cols="12" lg="4" md="6" sm="6" xs="12" class>
          <v-skeleton-loader
      class="mx-auto"
      max-width="300"
      type="card"
      v-if="loaded == false"
    ></v-skeleton-loader>
          <v-card v-if="loaded" min-height="410" color="white" min-width="260" class="rounded-t-lg no-gutters">
        ...
        <v-col v-for="(item,j) in items[i]" :key="j" cols="6" lg="4" sm="4" xs="4">
                <v-card elevation="1" @click="popUp(item)">
                  ...
                  <v-card-text class="text-center pa-1 cursr bold">{{ item.productName }}</v-card-text>
                              ...
                              <v-card-actions ><v-btn text outlined color="green-sea-btn2" class="ma-auto text-center"> See More</v-btn></v-card-actions>
          </v-card>
        </v-col>
      </v-row>
